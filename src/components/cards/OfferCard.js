...
const SingleCard = ({offCount, title, description, image, backgroundColor}) => {
...
const OfferCard = ({offers, onPress, styles: extraStyles}) => {
	...
	return (
		<Pressable onPress={onPress} style={{position: "relative"}}>
			<ScrollView style={[styles.container, extraStyles]} showsHorizontalScrollIndicator={false}
				...
				{offers.map((offer, index) => <SingleCard {...offer} key={index}/>)}
			</ScrollView>

			{/* Slider Dots */}
			{offers.length > 1 && <View style={styles.slider}>
					{offers.map((_, i) => <View key={i} style={activeSlide === i
